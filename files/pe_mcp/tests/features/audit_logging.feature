Feature: Audit trail attribution for tool invocations
  ...
  Scenario: Successful tool call emits audit event with outcome "success"
    ...
    Then an audit event is emitted with tool "puppet_node_lookup" and outcome "success"
  ...
  Scenario: Failed tool call emits audit event with outcome "error"
    ...
    When the MCP client calls puppet_node_lookup
    Then an audit event is emitted with tool "puppet_node_lookup" and outcome "error"

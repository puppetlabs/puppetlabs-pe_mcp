...
def _call_puppet_node_lookup(app: FastMCP, tel_state: dict[str, Any]) -> None:
    async def _run() -> Any:
...
@when(parsers.re(r'the MCP client calls puppet_pql_query with query "(?P<q>[^"]+)"'))
def _call_pql(q: str, app: FastMCP, tel_state: dict[str, Any]) -> None:
    async def _run() -> Any:
        async with Client(app) as client:
            return await client.call_tool("puppet_pql_query", {"query": q})
